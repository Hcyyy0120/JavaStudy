...
4.  所有的线程共享Java堆，但在堆中可以划分出一块线程私有的缓冲区（Thread Local Allocation Buffer，**TLAB**）
...
7.  在方法结束后（对象的引用出栈后），堆中的对象不会马上被移除，仅仅在垃圾收集的时候才会被移除。
...
2.  Java 8及之后堆内存**逻辑上**分为三部分：新生区+养老区+元空间
...
JVM内存（运行时数据区域就是我们常说的JVM的内存）会划分为堆内存和非堆内存，堆内存中也会划分为**年轻代**和**老年代**，而非堆内存则为**永久代**。
...
**设置两个Survivor区是否是为了解决内存碎片化存疑**

![image-20221025175950454](D:\JVM\JavaYouth\docs\JVM\assets\image-20221025175950454.png)
...
2.  当伊甸园的空间填满时，程序又需要创建对象，JVM的垃圾回收器将对伊甸园区进行垃圾回收（MinorGC/YoungGC），将伊甸园区中的不再被其他对象所引用的对象进行销毁。再加载新的对象放到伊甸园区。
...
2、当我们进行一次垃圾收集后，红色的对象将会被回收，而绿色的对象还被占用着，存放在S0(Survivor From)区。同时我们给每个对象设置了一个年龄计数器，经过一次回收后还存在的对象，将其年龄加 1。

3、同时Eden区继续存放对象，当Eden区再次存满的时候，又会触发一个Minor GC操作，此时GC将会把 ***Eden和Survivor From***中的对象进行一次垃圾收集，把存活的对象放到 Survivor To（S1）区，同时让存活的对象年龄 + 1
...
>
> 3、也就是说s0区和s1区在互相转换。（哪个区为空，就是to区）
...
1.  如果来了一个新对象，先看看 Eden 是否放的下？
    ...
    *   如果 Eden 放不下，则触发 YGC ，执行垃圾回收，看看还能不能放下？（将Eden区  可以被回收的对象回收/不可被回收的对象转移到to区  后，此时Eden区是空的，看整个Eden区的空间能否放下该新对象，若放不下，将对象直接放到老年区）（Eden和to  或  Eden+from和to  进行对象转移时，若to区放不下，会将对象直接放到老年区）
2.  将对象放到老年区又有两种情况：
...
[来自知乎]: https://www.zhihu.com/question/41922036/answer/93079526
...
- - Young GC：只收集young gen的GC
	...
	- Mixed GC：收集整个[young gen](https://www.zhihu.com/search?q=young+gen&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A93079526})以及部分old gen的GC。只有G1有这个模式
...
2.  JVM在进行GC时，并非每次都对上面三个内存区域一起回收的，大部分时候回收的都是指新生代。针对Hotspot VM的实现，它里面的GC按照回收区域又分为两大种类型：一种是部分收集（Partial GC），一种是整堆收集（Full GC）
...
3.  Minor GC会引发STW（Stop The World），暂停其它用户的线程，等垃圾回收结束，用户线程才恢复运行（虽然Minor GC比较频繁，但它速度快，所以对用户线程的影响不大）
...
2.  出现了Major GC，经常会伴随至少一次的Minor GC。（但非绝对的，在Parallel Scavenge收集器的收集策略里就有直接进行Major GC的策略选择过程）
    ...
    *   也就是在老年代空间不足时，会先尝试触发Minor GC（说法一：出现  大对象/相对大的对象  时，Eden区或to区放不下，需要放到old区，但此时old区空间不足，会先尝试进行YGC，看GC后Eden/to能否放下。说法二：不minor GC 怎么会有晋升，没有晋升major GC怎么会空间不足，是执行minor引起major），如果之后空间还不足，则触发Major GC
...
2.  **大对象直接分配到老年代**：尽量避免程序中出现过多的大对象（每次GC都会带来STW）

3.  **长期存活的对象分配到老年代**

4.  **动态对象年龄判断**：

	总体表征就是，一次YGC完毕之后却计算完新的年龄了，按年龄从小到大进行对象大小的累加，当加入某个年龄段后，累加和超过survivor区域TargetSurvivorRatio（默认50%）的时候，就将这个年龄作为新的晋升年龄，**下次**YGC时就以这个年龄作为对象晋升老年代的年龄。【为什么是下次而不是本次呢？因为如果本次YGC时就以这个年龄作为晋升标准，那survivor区永远也超不过默认值的50%了，更别说触发分配担保机制】
...
3.  为避免多个线程操作同一地址，需要使用**加锁等机制**，但会影响分配速度。
...
2.  多线程同时分配内存时，使用TLAB可以避免一系列的非线程安全问题，同时还能够提升内存分配的吞吐量（因为避免了加锁同步操作），因此我们可以将这种内存分配方式称之为**快速分配策略**。
...
>
> 2、比如一个线程的 TLAB 空间有 100KB，其中已经使用了 80KB，当需要再分配一个 30KB 的对象时，就无法直接在 TLAB 中分配，遇到这种情况时有两种处理方案：
>
> 1. 直接在堆内存中对该对象进行内存分配。（需要进行同步控制）
> 2. 废弃当前的 TLAB，重新申请 TLAB 空间再次进行内存分配。（需要进行同步控制）
...
首先，在执行任何一次Minor GC之前，JVM都会先检查一些老年代可用的内存空间，是否大于年轻代所有对象的总大小，为什么呢？因为最极端的情况下，可能年轻代Minor GC之后，所有对象都存活下来了，且s区已满，那岂不是年轻代所有对象全部进入老年代。
...
2.  JDK6 Update 24之后的规则变为**只要老年代的连续空间大于新生代对象总大小或者历次晋升的平均大小就会进行Minor GC**，**否则将进行Full GC**。即 HandlePromotionFailure永远为true
...
1.  随着JIT编译器的发展与**逃逸分析技术**逐渐成熟，**栈上分配、标量替换**优化技术将会导致一些微妙的变化，所有的对象都分配到堆上也渐渐变得不那么“绝对”了。
