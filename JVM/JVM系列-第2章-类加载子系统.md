...
2.  此方法不需定义，是javac编译器自动收集类中的所有**类变量**的赋值动作和静态代码块中的语句合并而来。也就是说，当我们代码中包含static变量的时候，就会有clinit方法（非也非也）
...
public class ClassInitTest {
   ...
   static{
       ...
       //System.out.println(number);报错：非法的前向引用。linking的prepare阶段只分配了内存空间，没声名变量，所以不能调用
   }

   /**
    * 1、linking的prepare: number = 0 --> initial: 20 --> 10
...
*   所以无论哪个线程先执行 DeadThread 类的加载，另外一个类都不会继续执行。
*   （一个类只会被加载一次）
...
<img src="https://npm.elemecdn.com/youthlql@1.0.8/JVM/chapter_002/0014.png" style="zoom:50%;" >



**ExtClassLoader**

<img src="https://npm.elemecdn.com/youthlql@1.0.8/JVM/chapter_002/0015.png" style="zoom: 33%;" >

**AppClassLoader**

<img src="https://npm.elemecdn.com/youthlql@1.0.8/JVM/chapter_002/0016.png" style="zoom: 33%;" >
...
4.  负责加载扩展类加载器和应用程序类加载器，并作为他们的父类加载器
...
5.  该类加载是程序中默认的类加载器，一般来说，**Java应用的类**都是由它来完成加载
6.  通过classLoader.getSystemclassLoader()方法可以获取到该类加载器
7.  我们可以直接使用这个类加载器。一般情况，如果我们没有自定义类加载器默认就是用这个加载器
...
系统类加载器一直向上委托,直到启动类加载器。启动类加载器发现是java.lang.String,是java包下的(**出于安全考虑，Bootstrap启动类加载器只加载包名为java、javax、sun等开头的类**),于是由启动类加载器加载这个类,最后new 的String就是Java核心API里的String。
