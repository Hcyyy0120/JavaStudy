...
1.  在默认情况下，通过System.gc()者Runtime.getRuntime().gc() 的调用，**会显式触发Full GC**，同时对老年代和新生代以及方法区进行回收，尝试释放被丢弃对象占用的内存。
...
**如何在GC发生时，如何检查所有线程都跑到最近的安全点停顿下来呢？**
...
2.  软引用（SoftReference）：在系统将要发生内存溢出之前，将会把这些对象列入回收范围之中进行第二次回收。如果这次回收后还没有足够的内存，才会抛出内存溢出异常。（内存不足则回收）
