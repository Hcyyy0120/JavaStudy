...
1.  解释器：当Java虚拟机启动时会根据预定义的规范对字节码采用**逐行**解释的方式**执行**，将每条字节码文件中的内容“翻译”为对应平台的***本地机器指令执*行**。
2.  JIT（Just In Time Compiler）编译器：就是虚拟机将源代码**一次性直接**编译成和本地机器平台相关的**机器语言**，**但并不是马上执行**。



编译型语言会通过编译器将源代码**一次性翻译**成可被该平台执行的**机器码**。

解释型语言会通过解释器**一句一句**的将代码解释（interpret）为**机器代码**后再执行。
...
**为什么说 Java 语言“编译与解释并存”？**（From JavaGuide）
