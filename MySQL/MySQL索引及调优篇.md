...
**概念：**[聚簇索引](https://so.csdn.net/so/search?q=聚簇索引&spm=1001.2101.3001.7020)就是按照每张表的**主键**构造一颗B+树，同时叶子节点中存放的就是整张表的行记录数据，也将聚集索引的叶子节点称为数据页。这个特性决定了索引组织表中数据也是索引的一部分，每张表只能拥有一个聚簇索引。

Innodb通过主键聚集数据，如果没有定义主键，innodb会选择非空的[唯一索引](https://so.csdn.net/so/search?q=唯一索引&spm=1001.2101.3001.7020)代替。如果没有这样的索引，innodb会隐式的定义一个主键来作为聚簇索引。
...
以非主键字段为搜索条件，eg：`select * from table where c2 = 4`时，就会在非聚簇索引中进行搜索

非聚簇索引的叶子结点并不包含所有数据，只含c2和主键，当要求搜索 * 的时候，就必须依据叶子结点中的主键值，再进行一次聚簇索引中的搜索。（回表）
...
https://blog.csdn.net/chanmufeng/article/details/123765399
...
原因1补充：由于Hash索引比较的是进行Hash运算之后的Hash值，所以它只能用于等值的过滤，不能用于基于范围的过滤，因为经							过相应的Hash算法处理之后的Hash值得大小关系，并不能保证和Hash运算前完全一样。
...
你能看出来在 B 树的搜索过程中，我们比较的次数并不少，但如果把数据读取出来然后在内存中进行比较，这个时间就是可以忽略不计的。而读取磁盘块本身需要进行 I/O 操作，消耗的时间比在内存中进行 比较所需要的时间要多，是数据查找用时的重要因素。 B 树相比于平衡二叉树来说磁盘 I/O 操作要少 ，在数据查询中比平衡二叉树效率要高。所以只要树的高度足够低，IO次数足够少，就可以提高查询性能 。
...
从 **功能逻辑** 上说，索引主要有 4 种，分别**普通索引、唯一索引、主键索引、全文索引**。 
...
举例：隐式创建，定义主键约束、外键约束或者唯一性约束，而不论创建哪种约束，在定义约束的同时相当于在指定列上创建了一个索引。
...
当使用该字段进行查询的时候，查询效率就会提高

select * from book where book_name = '一本书';
...
**最左前缀原则：**可以看到，查询id和name字段时，使用了名称为multi_idx(id,name,age)的索引，如果查询 (name, age) 组合或者单独查询name和age字段，会发现结果中possible_keys和key值为NULL, 并没有使用在t3表中创建的索引进行查询。b+树会优先比较id来确定下一步的所搜方向，如果id相同再依次比较name和age，最后得到检索的数据。

因为创建该索引而产生的b+树，是以id、name、age依次的大小来排序的（先id再name后age），当(name，age)这样的没有id的数据来的时候，b+树就不知道下一步该查哪个节点，因为建立搜索树的时候id就是第一个比较因子，必须要先根据id来搜索才能知道下一步去哪里查询。
...
比如student_info数据表（含100万条数据），假设我们想要查询 student_id=46778的用户信息。

select course_id,class_id,`name`,create_time,student_id from student_info where student_id = 46778;  **0.235s**

alter table student_info add index idx_sid(student_id);  **0.001s**

#### 3. 经常 GROUP BY 和 ORDER BY 的列

索引就是让数据按照某种顺序进行存储或检索，因此当我们使用 GROUP BY 对数据进行分组查询，或者使用 ORDER BY 对数据进行排序的时候，就需要对分组或者排序的字段进行索引 。如果待排序的列有多个，那么可以在这些列上建立组合索引 。

select student_id,count(*) as num from student_info group by student_id;  **0.19s**

drop index idx_sid on student_info;  **0.612s**
...
你能看到 SQL 查询效率有了提升，同时显示出来的 student_id 还是按照递增的顺序 进行展示的。这是因为**索引会对数据按照某种顺序进行排序，所以在去重的时候也会快很多。**
...
不论我们的查询语句有多复杂，里边儿包含了多少个表 ，到最后也是需要对每个表进行 单表访问 的，所 以MySQL规定EXPLAIN语句输出的每条记录都对应着某个单表的访问方法，该条记录的table列代表着该 表的表名（有时不是真实的表名字，可能是简称）。
...
* SUBQUERY
	...
	如果包含子查询的查询语句不能够转为对应的`semi-join`（多表连接）的形式，并且该子查询是**不相关子查询**，则该子查询的第一个`SELECT`关键字代表的那个查询的`select_type`就是`SUBQUERY`，比如下边这个查询：
...
* DEPENDENT SUBQUERY
  ...
  如果包含子查询的查询语句不能够转为对应的`semi-join`的形式，并且该子查询是**相关子查询**，则该子查询的第一个`SELECT`关键字代表的那个查询的`select type`就是`DEPENDENT SUBQUERY`

  注意的是，select _type为DEPENDENT SUBQUERY`的查询可能会被执行多次。
...
* DERIVED
  ...
  派生表，`是用于存储子查询产生的结果的临时表，这个`子查询`特指 `FROM 子句` 里的子查询，如果是出现在其它地方的子查询，就不叫这个名字了，所以本质上来说，派生表也是临时表。
...
* MATERIALIZED
  ...
  `物化表`，是用于存储子查询产生的结果的临时表，这个`子查询`特指 `WHERE` 子句中查询条件里的子查询。
...
* `index`
  ...
  当我们可以使用**索引覆盖**（索引覆盖可以不遵守最左前缀原则），但需要扫描全部的索引记录时，该表的访问方法就是`index`，比如这样：
  ...
  索引：idx_key_part(key_part1,key_part2,key_part3)
...
帮你检查`是否充分的利用了索引`，`值越大越好（与自身比较）`，主要针对于联合索引，有一定的参考意义。
...
不能对**索引列**进行函数运算，这也包括加减乘除的谓词运算，这也会使索引失效。
...
key_len为10，说明创建的联合索引中只使用了age和classId，name没有用到。

因为有**classId>20（范围条件）**，因此其右边的列不能正常使用。**关键在于索引的顺序，和where后的and连接的条件的顺序无关**
...
统一使用utf8mb4( 5.5.3版本以上支持)兼容性更好，统一字符集可以避免由于字符集转换产生的乱码。不 同的 `字符集` 进行比较前需要进行 `转换` 会造成索引失效。--->即2.5例所示
...
因为join是嵌套查询，左表拿出一条记录，在右表中查询相对应的记录，故要在右表（被驱动表）中建立索引
...
* 对于内连接来说：如果表的连接条件中只能有一个字段有索引，则**有索引的字段所在的表会被作为被驱动表**
...
1、**整体效率比较：INLJ > BNLJ > SNLJ**   索引嵌套>块嵌套>简单嵌套

2、永远用**小结果集**驱动**大结果集**（其本质就是减少外层循环的数据量） 谁有索引谁就是被驱动表，若都有/都没有，则小表驱动大表
...
若改为 select **age,classId** from student order by **age,classId**;  此时会使用索引，因为不需要回表操作

select ***** from student order by age,classId;  若使用索引，会需要进行回表操作，优化器认为不如一次性加载到内存再排序好
...
**过程四：order by 时规则不一致，索引失效（顺序错，不索引；方向（order by后的字段升降序要一致）反，不索引）**

索引默认创建是升序的

N，N，N，Y（backward index scan）
...
联合索引中只使用了age索引（1,2） 3没用，4用了（无法理解）
...
两个思路的核心都是往主键上靠（当查询的不是 * 时，就往索引列上靠）

## 8. 优先考虑覆盖索引

##### ==以下的例子中会和第10章中索引失效的案例有冲突，一切以优化器的决策为主，一切都是基于成本的考虑，索引失效的案例只是给我们提供一种判断==。
...
2.回表的时候可能是随机IO，使用覆盖索引就会减少了回表操作，也就减去了随机IO，使用了顺序IO
...
举例1：SELECT * FROM s1 WHERE key1 > 'z’ AND key1 LIKE '%a ';   （key：idx_key1，extra：using index condition）

**这个例子特殊在两个条件都是以 key1 列进行判断** 

不使用ICP：根据idx_key1索引去查询数据的时候，假设满足key1>'z'的数据有100条，因为是二级索引，且是  *  查询，因此需要进行回表操作，100条数据进行100次回表，在聚簇索引上根据key1 like '%a'进行查找，假设最终数据有10条，则进行100次回表查出10条数据。

使用ICP：根据idx_key1索引去查询数据的时候，假设满足key1>'z'的数据有100条，此时不去进行回表，而是继续进行key1 like '%a'条件的过滤，这时从100条数据中筛选出10条，再进行回表操作（可能会随机IO）。这样就大大减少了回表次数。

举例2：KEY ‘zip_last_first‘  ( zipcode , lastname , firstname)

SELECT * FROM people WHERE zipcode='000001' AND lastname LIKE '%张%' AND address LIKE'%北京市';

不使用ICP：只能使用zipcode这一个索引，zipcode='000001'筛选后进行回表，再进行lastname LIKE '%张%' AND address LIKE'%北京市'的筛选。

使用ICP：索引下推到lastname，zipcode='000001'筛选后继续进行lastname LIKE '%张%' 的筛选，再回表......
...
默认的，MySQL服务会将ANALYZE TABLE语句写到binlog中，以便在主从架构中，从服务能够同步数据。 可以添加参数LOCAL 或者 NO_WRITE_TO_BINLOG取消将语句写到binlog中。
